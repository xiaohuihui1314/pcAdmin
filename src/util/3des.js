import { base64Cucpay } from './base64';
// des
...
const des = (key, message, encrypt, mode, iv, padding) => {
  ...
  if (!encrypt) {
    // 濡傛灉鏄В瀵嗙殑璇濓紝瑙ｅ瘑缁撴潫鍚庡PKCS7 padding杩涜瑙ｇ爜锛屽苟杞崲鎴耻tf-8缂栫爜
    if (padding === 1) {
      // PKCS7 padding瑙ｇ爜
 ...
 * 返回的密文是十六进制字符串
 */
const triple_des_org = (key, message, encrypt, mode, iv, padding) => {
  let tmp = '';
  for (let i = key.length; i < 24; i++) {
    key += '0';
  }
...
const tripleDes = (key, message, encrypt, mode, iv, padding) => {
  let tmp = '';
  let deskey = base64Cucpay.decode(key);
  
  for (let i = deskey.length; i < 24; i++) {
    deskey += '0';
  }
